...
MainGame.prototype = {
	...
	create: function(){
		...
		this.txtScore = this.game.add.text(5,5,'Score: ' + this.game.score, style);
		this.txtTime = this.game.add.text(5, this.game.height - 5, 'Time left: ' + this.timeLeft, style);
	...
	checkTiles: function(){
		if(this.selectedArray[0].value === this.selectedArray[1].value){
			this.game.score++;
			this.timeLeft += 2;
			this.txtTime.text = 'Time left: ' + this.timeLeft;
	...
	clearTiles: function(){
		...
		this.txtScore.text = 'Score: ' + this.game.score;
